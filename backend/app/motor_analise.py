...
class ResultadoItem:
    ...
    cfop_st_divergente: bool = False   # True = produto sujeito a ST mas CFOP não é o esperado (5405)
    valor_total: Optional[float] = None   # valor do item na nota, usado no cálculo do Simples Nacional
...
def analisar_item(
    ...
    cfop: CFOP do item na nota, usado para validar se a nota está usando
        o código correto (5405) quando o produto é sujeito a ST
    numero_nf, chave_acesso: identificação da nota de origem, só para
        rastreabilidade nos avisos gerados (não afeta o cruzamento)
    valor_total: valor do item na nota, usado na apuração do Simples Nacional
    """
    ...
    # precisa estar com o CFOP correto (5405 — venda de mercadoria já
