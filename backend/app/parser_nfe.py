...
para cada item de cada nota: descrição do produto, NCM, CEST, CFOP, CST,
quantidade e valores.

Lida também com ZIP-dentro-de-ZIP (comum em exports de portais que
agrupam vários lotes num único arquivo) — procura .xml recursivamente
em qualquer .zip encontrado dentro do ZIP principal, até 2 níveis de
profundidade.
...
def processar_zip_nfe(caminho_zip: str) -> dict:
    ...
    Retorna um dict com:
        ...
        - extensoes_encontradas: contagem de extensões de arquivo vistas
          no ZIP (útil para diagnóstico quando nenhum .xml é encontrado)
    """
    notas = []
    erros = []
    extensoes_encontradas = {}

    def _processar_zip_bytes(conteudo_zip_bytes, profundidade=0):
        with zipfile.ZipFile(io.BytesIO(conteudo_zip_bytes), "r") as zf:
            for nome in zf.namelist():
                if nome.endswith("/"):
                    continue  # é só uma entrada de pasta, não um arquivo
                ext = nome.rsplit(".", 1)[-1].lower() if "." in nome else "(sem extensão)"
                extensoes_encontradas[ext] = extensoes_encontradas.get(ext, 0) + 1
                ...
                elif nome.lower().endswith(".zip") and profundidade < 2:
                    # ZIP dentro do ZIP — desce um nível e repete o processo
                    try:
                        _processar_zip_bytes(zf.read(nome), profundidade + 1)
                    except Exception as e:
                        erros.append({"arquivo": nome, "erro": f"Falha ao abrir ZIP interno: {e}"})

    with open(caminho_zip, "rb") as f:
        _processar_zip_bytes(f.read())

    total_xml_encontrados = extensoes_encontradas.get("xml", 0)

    return {
        "total_arquivos": total_xml_encontrados,
        "total_processados": len(notas),
        "total_erros": len(erros),
        "notas": [asdict(n) for n in notas],
        "erros": erros,
        "extensoes_encontradas": extensoes_encontradas,
    }
