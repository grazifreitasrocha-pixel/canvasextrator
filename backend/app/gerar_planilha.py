...
Estrutura em abas separadas:
    ...
    - "PIS-COFINS"      — detalhe de benefício e regime diferenciado de PIS/COFINS
    - "Avisos"          — divergências de CFOP em produtos sujeitos a ST
    - "Simples Nacional" — apuração segregada do DAS (só quando RBT12 é informado)
    - "Classificação IA" — parecer da IA sobre coerência da classificação (opcional)
"""
...
def _classificar_regime_diferenciado(regime_monofasico) -> str:
    """
    Classifica o regime de PIS/COFINS numa das três situações:
...
def _preencher_aba_avisos(ws, resultados):
    ...
    esperado (5405) para produtos identificados como sujeitos a
...
def _preencher_aba_simples_nacional(ws, apuracao):
    """
    apuracao: ResultadoApuracao (de simples_nacional.py) ou None.
    Mostra a segregação de receita e o DAS calculado por segmento.
    """
    ws.append(["Apuração do Simples Nacional — DAS segregado por tipo de receita"])
    ws["A1"].font = Font(bold=True, size=13)
    ws.append([])

    ws.append(["Anexo utilizado", apuracao.anexo])
    ws.append(["RBT12 (receita bruta últimos 12 meses)", f"R$ {apuracao.rbt12:,.2f}"])
    ws.append(["Faixa", apuracao.faixa])
    ws.append(["Alíquota nominal da faixa", f"{apuracao.aliquota_nominal*100:.2f}%"])
    ws.append(["Parcela a deduzir", f"R$ {apuracao.parcela_deduzir:,.2f}"])
    ws.append(["Alíquota efetiva cheia", f"{apuracao.aliquota_efetiva*100:.4f}%"])
    ws.append([])

    cabecalho = ["Segmento de receita", "Receita do mês (R$)", "Alíquota aplicada", "DAS do segmento (R$)"]
    linha_cabecalho = ws.max_row + 1
    ws.append(cabecalho)
    for col_idx in range(1, len(cabecalho) + 1):
        celula = ws.cell(row=linha_cabecalho, column=col_idx)
        celula.font = FONTE_CABECALHO
        celula.fill = FUNDO_CABECALHO

    linhas_dados = [
        ("Receita normal (sem ST, sem monofásico)", apuracao.receita_normal, apuracao.aliquota_efetiva, apuracao.das_receita_normal),
        ("Receita sujeita a ST-ICMS", apuracao.receita_st, apuracao.aliquota_efetiva_st, apuracao.das_receita_st),
        ("Receita em regime monofásico PIS/COFINS", apuracao.receita_monofasico, apuracao.aliquota_efetiva_monofasico, apuracao.das_receita_monofasico),
        ("Receita com ST + monofásico", apuracao.receita_st_e_monofasico, apuracao.aliquota_efetiva_st_e_monofasico, apuracao.das_receita_st_e_monofasico),
    ]
    for nome, receita, aliquota, das in linhas_dados:
        ws.append([nome, f"R$ {receita:,.2f}", f"{aliquota*100:.4f}%", f"R$ {das:,.2f}"])

    ws.append([])
    linha_total = ["TOTAL", f"R$ {apuracao.receita_total_mes:,.2f}", "-", f"R$ {apuracao.das_total:,.2f}"]
    ws.append(linha_total)
    for col_idx in range(1, len(linha_total) + 1):
        ws.cell(row=ws.max_row, column=col_idx).font = Font(bold=True)

    _ajustar_larguras(ws, [45, 22, 18, 22])
...
def gerar_planilha_resultado(resultados: list, caminho_saida: str, pareceres_ia: dict = None, apuracao_simples=None):
    """
    ...
    apuracao_simples: ResultadoApuracao opcional (de simples_nacional.py) —
        quando informado, gera a aba "Simples Nacional" com o DAS segregado.

    Gera a planilha em abas separadas: Resumo, ICMS, PIS-COFINS, Avisos e,
    quando aplicável, Simples Nacional e Classificação IA.
    """
    ...
    if apuracao_simples is not None:
        ws_simples = wb.create_sheet("Simples Nacional")
        _preencher_aba_simples_nacional(ws_simples, apuracao_simples)
...
if __name__ == "__main__":
    ...
    itens_exemplo = [
        {"descricao_produto": "PARAFUSO DE ACO INOX M6", "ncm": "73181500", "cfop": "5405", "valor_total": 10000.0},
        {"descricao_produto": "MEDICAMENTO GENERICO XYZ 500MG", "ncm": "30049099", "cfop": "5102", "valor_total": 5000.0},
        {"descricao_produto": "AGUA MINERAL 10L GALAO", "ncm": "22011000", "cfop": "5102", "valor_total": 3000.0},
        {"descricao_produto": "NOTEBOOK DELL I7 16GB", "ncm": "73181500", "cfop": "5102", "valor_total": 2000.0},
    ]
    ...
    apuracao = apurar_das_do_mes(resultados, rbt12=1_000_000.00, anexo="I")

    caminho = gerar_planilha_resultado(resultados, "/home/claude/sistema-fiscal/data/teste_final.xlsx", pareceres_simulados, apuracao)
